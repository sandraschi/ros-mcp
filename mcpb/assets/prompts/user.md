...
2. **ROS 2** installed on your system
3. Repository cloned: `git clone https://github.com/sandraschi/ros-mcp.git`
4. Dependencies installed: `cd ros-mcp && uv sync`
...
then opens the dashboard in your browser at http://localhost:11051.
...
```bash
uv run python -c "from ros_mcp.server import sim_status; print(sim_status())"
```
...
### Ports
- Backend API: 11050 (FastAPI REST + MCP HTTP endpoints)
- Frontend Dashboard: 11051 (Vite React SPA)
...
### 1. "ROS 2" not available from sim_status()
Ensure ROS 2 is installed and accessible. Check that the binary or Python module
...
```
If ports 11050 or 11051 are in use, kill the occupying processes and restart.
...
- Use topic_list() to load
- Use param_get() to verify it is present
...
|---|---|---|---|
| ros_status() | Sim | ... | {success, ...} |
| topic_list() | Sim | ... | {success, ...} |
| topic_echo() | Sim | ... | {success, ...} |
| topic_pub() | Sim | ... | {success, ...} |
| service_list() | Sim | ... | {success, ...} |
| service_call() | Sim | ... | {success, ...} |
| node_list() | Sim | ... | {success, ...} |
| param_get() | Sim | ... | {success, ...} |
| param_set() | Sim | ... | {success, ...} |
| launch() | Sim | ... | {success, ...} |
| stop_launch() | Sim | ... | {success, ...} |
| bag_record() | Sim | ... | {success, ...} |
| bag_play() | Sim | ... | {success, ...} |
| list_jobs() | Sim | ... | {success, ...} |
| agentic_ros_workflow() | AI | ... | {success, ...} |

## Known Issues

1. **First launch latency**: The first ROS 2 launch may be significantly slower due to
